...
dSum1 <- d1 %>% group_by(State, CaseType) %>%
  summarise(TotalCases = sum(Cases))
...
###
# no of case types by state - 26/06/2016
###

table(dSum1$State,dSum1$CaseType)
a <- (as.data.frame(table(dSum1$State,dSum1$CaseType)))
a <- (as.data.frame(table(dSum1$State)))
a <- count(dSum1,State)
names(a)
names(a) <- c("State","NumOfCaseType")
View(a)
sum(a$NumOfCaseType)
dim(dSum1)

View(filter(a,Var1 == "Andhra Pradesh"))
count()

write.csv(a,"../dataForD3/StateNumCaseType.csv",row.names=FALSE)

###
#
###
dSum <- filter(dSum1,State %in% c("Chhattisgarh","Andhra Pradesh","Haryana","Jharkhand","Punjab","Himachal Pradesh"))
dSum <- filter(dSum1,State == "Chhattisgarh")
...
write.csv(p,"../dataForD3/ChattisgarhCaseType.csv",row.names=FALSE)
write.csv(p,"../dataForD3/HighDensityStateCaseType.csv",row.names=FALSE)
