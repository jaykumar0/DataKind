...
p1 <- as.data.frame(table(q$before_honourable_judges,q$petitioner_advocate))
View(p1  %>% arrange(desc(Freq)))

p11 <- filter(p1,Freq > 1)
View(p11)
names(p11) <- c("source","target","value")
write.csv(p11,"../dataForD3/medakNWJudgeAndPetAdv.csv",row.names=FALSE)

table(p11$source)
table(p11$target)
str(p11)

p11$source <- droplevels(p11$source)
p11$target <- droplevels(p11$target)

table(p11$source,p11$target)
table(p11$target,p11$source)
p11$sourceN <- as.integer(p11$source)
p11$targetN <- as.integer(p11$target)
View(p11)

p11$sourceN <- p11$sourceN -1
p11$targetN <- p11$targetN
View(p11)
...
x <- "{ \"source\" : }"
x  
x <- "{ \"source\" : p11$sourceN[0]}"
x  
p11$sourceN1 <- paste('{ "source" : ',p11$sourceN,sep="")

x <-  "The boy said, \"Hello World!\""
x
#############################################################
#############################################################
#############################################################
...
pr1 <- as.data.frame(table(q$petitioner_advocate,q$respondent_advocate))
View(pr1  %>% arrange(desc(Freq)))
